...
export function useClaimValidation(score: number): ClaimValidation {
  ...
  if (contractBalance && estimatedReward) {
    if (contractBalance < estimatedReward) {
      ...
      warnings.push(`Contract has ${formatEther(contractBalance)} JUMP, needs ${formatEther(estimatedReward)} JUMP`);
    }
  }
  ...
  return {
    ...
    timeUntilClaim: lastClaimTime && cooldownPeriod
      ? Math.max(0, Number(lastClaimTime) + Number(cooldownPeriod) - Math.floor(Date.now() / 1000))
      : undefined,
  };
}
