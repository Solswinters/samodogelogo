...
interface UseMultiplayerProps {
  ...
  onPositionUpdated?: (playerId: string, position: any) => void;
  ...
  onGameStarted?: () => void;
  onGameOver?: (data: { winnerId: string; scores: any[] }) => void;
}
...
export function useMultiplayer(props: UseMultiplayerProps) {
  ...
  useEffect(() => {
    ...
    socketInstance.on("room-joined", (data: { roomId: string; playerId: string; player: Player }) => {
    ...
    socketInstance.on("room-state", (data: { players: any[]; obstacles: Obstacle[]; gameStarted: boolean }) => {
  ...
  const updatePosition = useCallback((position: { y: number; velocityY: number; isGrounded: boolean }) => {
    if (socket) {
  ...
  const syncObstacles = useCallback((obstacles: Obstacle[]) => {
    if (socket && isHost) {
